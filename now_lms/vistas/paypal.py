    ...
    try:
        ...
        if not all([order_id, payer_id, course_code, amount]):
            missing_fields = [field for field, value in {
                'orderID': order_id, 'payerID': payer_id, 
                'courseCode': course_code, 'amount': amount
            }.items() if not value]
        ...
        if abs(verified_amount - expected_amount) > amount_tolerance:
            logging.error(f"Payment amount mismatch for course {course_code}, user {current_user.usuario}: expected {expected_amount}, got {verified_amount}")
            return jsonify({'success': False, 'error': f'Payment amount mismatch: expected {expected_amount}, received {verified_amount}'}), 400
...
def resume_payment(payment_id):
    ...
    try:
        ...
        pago = database.session.query(Pago).filter_by(
            id=payment_id,
            usuario=current_user.usuario,
            estado='pending'
        ).first()
...
def payment_status(course_code):
    ...
    try:
        ...
        enrollment = database.session.query(EstudianteCurso).filter_by(
            usuario=current_user.usuario,
            curso=course_code,
            vigente=True
        ).first()
        ...
        payments = database.session.query(Pago).filter_by(
            usuario=current_user.usuario,
            curso=course_code
