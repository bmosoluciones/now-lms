...
"""
        return FileStorage(
            stream=io.BytesIO(vtt_content.encode('utf-8')),
            filename="test.vtt",
    ...
    def test_curso_recurso_model_has_subtitle_vtt(self, full_db_setup):
        ...
        with full_db_setup.app_context():
            ...
            audio_resource = database.session.execute(
                select(CursoRecurso).filter_by(tipo="mp3")
            ).scalar_one_or_none()
            ...
            if audio_resource:
                ...
                saved_resource = database.session.execute(
                    select(CursoRecurso).filter_by(id=audio_resource.id)
                ).scalar_one()
