...
class FeatureValidator:
            ...
            for path in health_paths:
                try:
                    ...
                    if response.status_code == 200:
                        self.log_result(
                            "Health endpoint responds with HTTP 200", True, f"Path: {path}, Status: {response.status_code}"
                        )
                        return True
                except:
                    continue

            self.log_result("Health endpoint responds with HTTP 200", False, "No health endpoint found at common paths")
            return False
        except Exception as e:
    ...
    def test_login_valid_credentials(self):
        ...
        try:
            ...
            if login_page.status_code != 200:
                self.log_result("Login with valid credentials", False, f"Login page not accessible: {login_page.status_code}")
                return False

            # Attempt login
            login_data = self.admin_credentials.copy()
            response = self.session.post(urljoin(self.base_url, "/user/login"), data=login_data, allow_redirects=False)
    ...
    def test_login_invalid_credentials(self):
        ...
        try:
            ...
            response = self.session.post(urljoin(self.base_url, "/user/login"), data=login_data, allow_redirects=True)

            # Should stay on login page with error message
            passed = response.status_code == 200 and (
                "error" in response.text.lower() or "invalid" in response.text.lower() or "incorrect" in response.text.lower()
            )
            self.log_result(
    ...
    def test_blog_functionality(self):
        ...
        try:
            ...
            self.log_result("Basic blog functionality", passed, f"Blog page status: {blog_response.status_code}")
            return passed
        except Exception as e:
    ...
    def test_multiple_database_support(self):
        ...
        try:
            ...
            self.log_result("Database support (SQLite default)", passed, f"Database file exists: {passed}")
            return passed
        except Exception as e:
    ...
    def test_static_assets(self):
        ...
        try:
            ...
            self.log_result("Static assets load correctly", passed, f"Assets loaded: {loaded_assets}/{len(static_paths)}")
            return passed
        except Exception as e:
    ...
    def test_forms_load(self):
        ...
        try:
            ...
            self.log_result("Key forms load without errors", passed, f"Forms loaded: {loaded_forms}/{len(form_pages)}")
            return passed
        except Exception as e:
    ...
    def test_navigation_without_500_errors(self):
        ...
        try:
            ...
            self.log_result(
                "Navigation without 500 errors", passed, f"Pages without 500 errors: {no_errors}/{len(navigation_paths)}"
            )
            return passed
        except Exception as e:
    ...
    def test_cache_functionality(self):
        ...
        try:
            ...
            passed = (
                response1.status_code == 200 and response2.status_code == 200 and second_load_time <= first_load_time * 1.5
            )
            self.log_result(
                "Cache works (second load faster)", passed, f"First: {first_load_time:.3f}s, Second: {second_load_time:.3f}s"
            )
            return passed
        except Exception as e:
