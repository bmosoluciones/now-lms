...
@cache.cached(timeout=60, key_prefix=cache_key_with_query_string)  # type: ignore[arg-type]
def blog_index() -> str:
...
def admin_create_post() -> str | Response:
    ...
    if current_user.tipo == "admin":
        ...
        form.status.choices = [("draft", _("Borrador")), ("pending", _("Pendiente"))]
...
def approve_post(post_id: int) -> Response:
    ...
    flash(_("Entrada '{title}' aprobada y publicada.").format(title=post.title), "success")
    return redirect(url_for(ROUTE_BLOG_ADMIN_INDEX))
...
def ban_post(post_id: int) -> Response:
    ...
    flash(_("Entrada '{title}' ha sido baneada.").format(title=post.title), "warning")
    return redirect(url_for(ROUTE_BLOG_ADMIN_INDEX))
...
def delete_tag(tag_id: str) -> Response:
    ...
    flash(_("Etiqueta '{name}' eliminada.").format(name=tag.name), "info")
    return redirect(url_for("blog.admin_tags"))
