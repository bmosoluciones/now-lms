...
class TestHTMLPreformattedDescriptions:
    ...
    def test_recurso_model_has_html_field(self, session_basic_db_setup):
        ...
        with session_basic_db_setup.app_context():
            ...
            recurso = Recurso(nombre="Test Resource", codigo="TEST001", descripcion="Test description", tipo="text")
    ...
    def test_html_preformatted_flag_can_be_set(self, session_basic_db_setup):
        ...
        with session_basic_db_setup.app_context():
            ...
            # Retrieve and verify
            saved_recurso = (
                database.session.execute(database.select(Recurso).filter(Recurso.codigo == "HTML001")).scalars().first()
            )
    ...
    def test_mixed_resources_different_flags(self, session_basic_db_setup):
        ...
        with session_basic_db_setup.app_context():
            ...
            html_saved = (
                database.session.execute(database.select(Recurso).filter(Recurso.codigo == "HTML002")).scalars().first()
            )
            md_saved = database.session.execute(database.select(Recurso).filter(Recurso.codigo == "MD002")).scalars().first()
    ...
    def test_html_field_defaults_to_false(self, session_basic_db_setup):
        ...
        with session_basic_db_setup.app_context():
            ...
            recurso = Recurso(nombre="Default Resource", codigo="DEF001", descripcion="Default description", tipo="text")
            database.session.add(recurso)
            database.session.commit()

            # Verify default is False
            saved_recurso = (
                database.session.execute(database.select(Recurso).filter(Recurso.codigo == "DEF001")).scalars().first()
            )
