...
| Default custom pages can be resynchronised after a language change — `paginas_predeterminadas()` / `sincronizar_paginas_predeterminadas()` in `now_lms/db/initial_data.py` plus `lmsctl database sync-default-pages` | **Core source** (seeding + CLI) | Genuinely upstream's bug, not a product decision: the seeder is create-if-absent and `initial_setup` never re-runs, so About Us and Privacy Policy are frozen in the seed language forever. Offer upstream and retire from the fork once accepted. |
